...
def show_sections_management(db: Database, user, lang: str):
    ...
    # Delete confirmation for coordinator
    if st.session_state.get('show_delete_coord_confirm', False) and existing_coord:
    ...
    if filtered_sections:
        # Create table header
        header_cols = st.columns([0.8, 1.2, 0.8, 0.8, 0.8, 0.6, 0.8, 1.5, 1.5])
        with header_cols[0]:
            st.markdown("**Sec #**")
        with header_cols[1]:
            st.markdown("**Course**")
        ...
        for section in filtered_sections:
            row_cols = st.columns([0.8, 1.2, 0.8, 0.8, 0.8, 0.6, 0.8, 1.5, 1.5])
            ...
            with row_cols[2]:
                st.markdown(f"{section.get('course_version', '1')}")
            with row_cols[3]:
                ...
                if not selected_program_id:
                    ...
                    for s in sections:
                        if (s.get('course_id') == selected_course.get('course_id') and
                            ...
                            s.get('section_number') == new_section_number):
